...
// ADMIN LIST MANAGER
// ========================================
// Manages admin list loading and emoji display functionality
// The emoji from admins.json will be displayed next to admin names
const AdminListManager = {
  ...
  async loadAdminList() {
    try {
      ...
      // Load emoji from the data
      this.adminEmoji = data.emoji || "👑";
      console.log(`🛡️ Admin emoji loaded: ${this.adminEmoji}`);

      if (data.updatedAt) console.log(`Admin list updatedAt: ${data.updatedAt}`);
      console.log(`Loaded ${admins.length} admins with emoji: ${this.adminEmoji}`);
      
      // Test emoji functionality
      this.testEmojiDisplay();
    } catch (error) {
  ...
  // Debug function to test emoji display
  testEmojiDisplay() {
    if (this.adminEmoji) {
      console.log(`🎯 Testing emoji display with: ${this.adminEmoji}`);
      console.log(`📋 Current admin list:`, CONFIG.adminList);
      console.log(`🔍 Admin regex:`, this.adminRegex);
    } else {
      console.log(`❌ No emoji loaded`);
    }
  }
};
      ...
      elements.forEach(element => {
        ...
        for (const phrase of patterns) {
          if (element.textContent.includes(phrase)) {
            ...
            // Add emoji to admin names in log messages
            if (color === COLORS.adminAction && AdminListManager.adminEmoji) {
              const emoji = AdminListManager.adminEmoji;
      ...
      if (regex) {
        elements.forEach(element => {
          ...
          if (regex.test(element.textContent)) {
            element.style.color = COLORS.adminName;
            // Add emoji if not already present
            if (emoji && !element.textContent.includes(emoji)) {
              element.innerHTML = `${emoji} ${element.textContent}`;
            }
          }
        });
